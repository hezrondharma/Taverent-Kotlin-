...
import com.example.taverent.databinding.ActivityProfileSewaBinding
import org.json.JSONArray

class ProfileSewa : AppCompatActivity() {
    private lateinit var binding: ActivityProfileSewaBinding
    var WS_HOST = ""
    var pemiliks: ArrayList<Pemilik> = ArrayList()
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityProfileSewaBinding.inflate(layoutInflater)
        ...
        binding.btnsimpan.setOnClickListener {
            ...
            var id_pemilik = intent.getStringExtra("id_pemilik").toString()
            if(nama_lengkap != "" || passwords!= ""||no_telp!= "") {
                val strReg = object : StringRequest(
                    Method.POST, "$WS_HOST/pemilik/update",
                    Response.Listener {
